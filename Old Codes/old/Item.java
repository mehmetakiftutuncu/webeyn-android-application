...
public class Item
{
	...
	/** Tag in RSS feed corresponding to {@link Item#content} */
	...
	public void setContent(String content)
	{
		...
		int startPoint = content.indexOf("</div>");
		if(startPoint < 0)
		{
		...
		else
		{
			...
			this.content = content.substring(startPoint + 6); // + 6 is because </div> is 6 characters long (Very elle elegan as you can see :))
		}
		...
		startPoint = content.indexOf("src=\"");
		if(startPoint < 0)
		{
			return;
		}
		...
		int endPoint = content.indexOf("\"", startPoint);
		if(endPoint < 0)
		{
			return;
		}
		...
		setImageLink(content.substring(startPoint, endPoint));
	}
	...
	public String toString()
	{
		return String.format("Item[\ntitle=%s\ncontent=%s\nlink=%s\nimageLink=%s\npublishDate=%s\ncreator=%s\ncategories=%s\nnumberOfComments=%s\n]",
						title, content, link, imageLink, publishDate, creator, categories, numberOfComments);
	}	
}